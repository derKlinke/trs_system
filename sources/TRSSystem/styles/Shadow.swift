...
struct ShadowViewModifier: ViewModifier {
    ...
    func body(content: Content) -> some View {
        let resolvedColor = themeManager.color(for: colorElement).withOpacity(opacity).color
...
extension View {
    public func shadow(_ baseSize: TRSSizes, x: CGFloat = 0, y: CGFloat = 0,
        ...
        self.modifier(ShadowViewModifier(baseSize: baseSize, x: x, y: y, colorElement: colorElement,
                                         opacity: opacity))
    }
